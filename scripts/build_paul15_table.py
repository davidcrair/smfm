...
Reads logs/paul15_power_law_linear_b512_split{42..46}.log, parses CHAINED
...
surf_latex/final_report/paul15_spectral_alpha_sweep.tex.
...
LOGS = [LOG_DIR / f"paul15_power_law_linear_b512_split{s}.log" for s in SPLITS]
...
def main():
    print("Parsing 5 paul15 split logs...")
    chained_med = collect(r"CHAINED EVAL \[MMD\^2\] \(1 seed\)")
    chained_otpfm = collect(r"CHAINED EVAL \[MMD\^2_otpfm\] \(1 seed\)")
    persegm_med = collect(r"PER-SEGMENT EVAL \[MMD\^2\] \(1 seed\)")
    persegm_otpfm = collect(r"PER-SEGMENT EVAL \[MMD\^2_otpfm\] \(1 seed\)")
    ...
    table = (
        ...
        + r"\centering" + "\n"
        + r"\caption{}" + "\n"
        + r"\label{tab:paul15_spectral_alpha_sweep}" + "\n"
    ...
    out_path = PROJ / "surf_latex" / "final_report" / "paul15_table.tex"
    ...
    for label, sig_dict in [("MMD²", paired_wilcoxon(r"CHAINED EVAL \[MMD\^2\] \(1 seed\)")),
                            ("MMD²_otpfm", paired_wilcoxon(r"CHAINED EVAL \[MMD\^2_otpfm\] \(1 seed\)"))]:
        print(f"\n  -- {label} --")
