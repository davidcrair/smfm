...
# HVG ambient space** (no sphere encoding) on pancreas. Mirrors the Paul15
# log1p sweep. Reports MMD^2 / MMD^2_otpfm / SWD in both 2000-dim ambient
# and 50-dim PCA latent.
set -euo pipefail
...
run_split() {
  local split="$1"
  local log="logs/pancreas_power_law_linear_log1p_b512_split${split}.log"
  local outdir="outputs/pancreas_power_law_linear_log1p_b512_split${split}"

  if [[ -f "$log" ]] && grep -q "PER-SEGMENT EVAL" "$log"; then
    if [[ "${N_SEEDS}" == "1" ]] || grep -q "±" "$log"; then
      echo "=== pancreas log1p split ${split}: log already complete for n_seeds=${N_SEEDS}, skipping ==="
      return 0
    fi
  fi
  ...
  echo "=== pancreas log1p ambient sweep, batch=512, n_seeds=${N_SEEDS}, split=${split} ==="
  "${PY}" -u -m surf.train \
    data=pancreas \
    ...
    2>&1 | tee "${log}"
  echo "=== pancreas log1p split ${split} done ==="
}
