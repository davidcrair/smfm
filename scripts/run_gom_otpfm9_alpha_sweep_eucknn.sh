#!/usr/bin/env bash
# GoM 9-stage OTP-FM hold-out alpha-sweep with **Euclidean kNN** (proper
# metric for 2D spatial data; the default cosine kNN groups cells by
# angle-from-origin which is geometrically arbitrary in 2D).
set -euo pipefail
...
# Use Euclidean kNN for the spectral Laplacian -- correct metric for 2D
# spatial data like GoM particles.
export SMFM_KNN_METRIC=euclidean

run_split() {
  local split="$1"
  local log="logs/gom_otpfm9_alpha_sweep_eucknn_split${split}.log"
  local outdir="outputs/gom_otpfm9_alpha_sweep_eucknn_split${split}"

  if [[ -f "$log" ]] && grep -q "PER-SEGMENT EVAL" "$log"; then
    if [[ "${N_SEEDS}" == "1" ]] || grep -q "±" "$log"; then
      echo "=== GoM-9 alpha-sweep+EucKNN split ${split}: log already complete, skipping ==="
      return 0
    fi
  fi
  ...
  echo "=== GoM-9 OTP-FM alpha-sweep + Euclidean kNN, batch=512, n_seeds=${N_SEEDS}, split=${split} ==="
  ...
  "${PY}" -u -m surf.train \
    ...
    2>&1 | tee "${log}"
  echo "=== GoM-9 alpha-sweep+EucKNN split ${split} done ==="
}
