#!/usr/bin/env bash
# Power-law spectral OT cost sweep, Linear/Euclidean trainer, **raw log1p
# HVG ambient space** (no sphere encoding), use_ema=false, on bonemarrow.
# Sanity check whether the -44.3% sphere-encoded BM headline holds up
# when the sphere normalization is removed.
set -euo pipefail
...
run_split() {
  local split="$1"
  local log="logs/bonemarrow_power_law_linear_log1p_b512_split${split}.log"
  local outdir="outputs/bonemarrow_power_law_linear_log1p_b512_split${split}"

  if [[ -f "$log" ]] && grep -q "PER-SEGMENT EVAL" "$log"; then
    if [[ "${N_SEEDS}" == "1" ]] || grep -q "±" "$log"; then
      echo "=== bonemarrow log1p split ${split}: log already complete for n_seeds=${N_SEEDS}, skipping ==="
      return 0
    fi
  fi
  ...
  echo "=== bonemarrow log1p ambient sweep, batch=512, n_seeds=${N_SEEDS}, split=${split} ==="
  "${PY}" -u -m surf.train \
    data=bonemarrow \
    ...
    2>&1 | tee "${log}"
  echo "=== bonemarrow log1p split ${split} done ==="
}
